...
export async function POST(req: NextRequest) {
  ...
  if (firstReward) {
    // As perguntas já respondidas foram creditadas uma a uma em /api/missions/check
    // (credited_ids). Aqui só reconcilia o que, por algum motivo, ainda não entrou —
    // evita creditar tudo de novo (dobrado) e evita perder algo se um check() falhou.
    const creditedIds: string[] = existing?.credited_ids ?? [];
    const alreadyCreditedSats = graded.details
      .filter((d) => creditedIds.includes(d.id))
      .reduce((sum, d) => sum + d.sats, 0);
    const remaining = graded.satsEarned - alreadyCreditedSats;

    satsCredited = graded.satsEarned;
    xpGained = graded.correctCount * 2;
    const nextBalance = satsBalance + Math.max(0, remaining);

    const { data: updated, error: creditError } = await supabaseAdmin
      .from("users")
      .update({
        xp: (user?.xp ?? 0) + xpGained,
        sats_balance: nextBalance,
      })
      .eq("id", session.userId)
      .select("sats_balance")
      .single();
    ...
    if (creditError || updated?.sats_balance == null) {
      console.error("[missions/submit] falha ao creditar sats_balance", {
    ...
    console.log("[missions/submit] conclusão creditada", {
      userId: session.userId,
      slug,
      satsCredited,
      alreadyCreditedSats,
