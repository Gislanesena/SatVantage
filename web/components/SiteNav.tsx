...
type SiteNavProps = {
  onStart?: () => void;
  /** mentor: logo + tema + sair · dash: só logo + tema (sair fica no menu do dash) */
  ...
  return (
    <header className="sv-nav">
      <div className={`sv-nav-inner${isCompact ? " sv-nav-inner--mentor" : ""}`}>
        ...
        {!isCompact && (
          <nav className="sv-nav-links" aria-label="Seções">
            {NAV_LINKS.map((link) => (
      ...
      {!isCompact && menuOpen && (
        <div className="sv-nav-drawer" role="dialog" aria-label="Menu">
          {NAV_LINKS.map((link) => (
