...
export default function BtcMarket({ variant = "card" }: BtcMarketProps) {
  ...
  return (
    <section
      ...
      aria-label="Preço do Bitcoin"
    >
      <div className="sv-btc-top">
        <p className="sv-btc-label">Bitcoin ao vivo</p>
        {data && (
      ...
      {data ? (
        <>
          <div className="sv-btc-price-row">
            <p className="sv-btc-price">{fmtBrl(data.priceBrl, 0)}</p>
            <p className="sv-btc-sub">
              US$ {data.priceUsd.toLocaleString("en-US", { maximumFractionDigits: 0 })}
            </p>
          </div>

          <div className="sv-btc-ranges" role="tablist" aria-label="Período do gráfico">
            {RANGES.map((r) => (
              <button
                key={r.id}
                type="button"
        ...
        <p className="sv-btc-sub">
          {error ? "Mercado indisponível agora" : loading ? "Carregando preço…" : "Sem dados"}
        </p>
      )}
    </section>
  );
